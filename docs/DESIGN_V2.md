...
- **Tiered gate on write** — the LLM is the last resort, not the first:
  ...
  **Live-calibration revision:** the first live run proved the point about porting thresholds — a real-phrasing contradiction fell below the hand-picked 0.70 band and silently ADDed. Current rule: on **decider-available** paths, anything above a low floor (`T_DECIDE_FLOOR` 0.50) is adjudicated by the decision model (its prompt handles "unrelated → ADD" / "same → NONE"); hard thresholds only gate the no-decider paths, which stay conservative. The gate also emits one content-free log line per write (`memory gate: top_sim=… tier=… action=…`) so thresholds are tuned from observed data, not guesses.
