...
- **Tiered gate on write** — the LLM is the last resort, not the first:
  ...
  **Live-calibration revision (final, measured):** the acceptance runs proved the porting-thresholds warning *twice* — the hand-picked 0.70 band missed the real contradiction, and so did the research-informed 0.50 floor. The gate's telemetry then delivered ground truth: on this embedder (3-large @ 1536), the real "three bullets → five bullets" contradiction measures **top_sim = 0.309**, and an unrelated fact measured 0.400 with the decider correctly choosing ADD. Final rule: on **decider-available** paths, anything above `T_DECIDE_FLOOR = 0.30` (live-calibrated) is adjudicated by the decision model; hard thresholds only gate the no-decider paths, which stay conservative. The gate emits one content-free log line per write (`memory gate: top_sim=… tier=… action=…`) — the mechanism that made this calibration possible and keeps future tuning data-driven.
