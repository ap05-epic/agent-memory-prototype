...
| §edits / recall | the block is appended to the agent's **instructions** | **Superseded (W3).** Recalled memory left the instruction channel entirely: it is now a **separate input-list item** — a fenced user-role message ahead of the user's message, on fresh turns only (resume/RunState paths untouched). The v1 `memory_block` kwarg was deleted from `build_agent`, returning the adapter to the team's original code. A probe run *before* any edit disproved the assumption that input items are not persisted — they are — so `session_filter.MemoryItemFilterSession` wraps the SDK session and drops the injected item on the way to storage. Live receipt: zero rows containing `<user_memory>` in `agent_messages`, with recall and the 🧠 indicator working normally. |
| §extraction | post-turn extraction is **fire-and-forget**; a dead process loses that turn's learning | **Superseded (W4).** Eligible turns now enqueue a durable job in `agent_memory_outbox` (Alembic revision `6f4f8e6f7f55`) at **both** terminal sites — inside `RESPONSE_COMPLETED` and at the fallthrough `RUN_COMPLETED` that structured-output turns hit, which is the completion path the review flagged as uncovered. `MemoryExtractionWorker` (modelled on `ProfileHealthMonitor`; started in the lifespan, stopped before the databases close) drains it: claim in a short leased transaction → extract with **no session held** → finalise in a short transaction. Retries back off to a five-attempt cap; a dead worker's lease expires and the row returns. If the enqueue itself fails, the old fire-and-forget path still runs, so behaviour never regresses. Live receipt: work enqueued, server killed, backlog drained on boot (`memory outbox: processed=4 failed=0`), recall then recited the new memory. |
...
6. **Injection boundary (W3):** recalled memory moved to the model input channel with a session wrapper keeping it out of stored history, above.
7. **Durable extraction (W4):** the outbox table, the leased worker, and the restart-survival proof, above.

## What's next (merge candidate 2, remaining)

Governed memory APIs — view / delete / forget / disable per user — with audit events on the harness governance rails, retention windows and a scheduled hard purge (W2) · console tenant plumbing, which is what re-enables memory for console traffic. Then the deferred long-term stage: consolidation into `agent_memory_user_models` (the "living user profile").
