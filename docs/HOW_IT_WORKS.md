...
2. **Recall injection** (~12 lines across two files): `stream_turn` (async, has the user) fetches + renders the memory block when the flag is on and passes it into `build_agent` as a new optional `memory_block: str | None = None` kwarg; `build_agent` appends it after its own instruction resolution — the same optional-override pattern its `instructions` parameter already uses. None (all existing callers) = byte-identical behavior.
...
4. **Tool wiring**: the pre-built `save_memory` SDK tool is registered once, right after `ToolRegistry` is constructed in `agent_factory/api/app.py`, via `register_custom_tool`. An agent gets the tool by listing `save_memory` under `tools.function_tools` in its profile yaml — so *which agents get the tool* rides the existing per-profile tool plan (`plan_tools` resolves custom-registered names; recon-verified). (The "native" alternative — a builder like `_build_workspace_sdk_tools` — is the documented productionization path.)
