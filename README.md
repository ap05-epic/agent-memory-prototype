...
- **★ MERGE-CANDIDATE 1 — MR opened, now Draft** (team direction: keep building on the branch rather than merge in stages) — snapshot branch `feature/agentmemory-mc1` (from `938de17`, plus a commit removing the docs bundle so the diff is code and tests only; net 26 files, no doc paths, no W3/W4). Everything needed to open it: `docs/MC1_PACKAGE.md` — MR description, findings→receipts crib sheet, message to send, and the GitLab steps for targeting `dev`.
...
- [x] **W2 governed memory COMPLETE** — commit `8fd9dcf`. Endpoints, audit table (revision `4f743f1f0d2d`), per-user opt-out, retention worker. Live-verified isolation: another user gets an empty list, another tenant gets an empty list, and a query parameter cannot override the header identity; the audit trail records every mutation and contains no memory text.
- [x] **Merged with dev** — merge commit `f9447ff`, branch now **0 behind / 13 ahead**. Two conflicts at the integration points; dev had unified turn completion, so our early-return terminal block was removed and extraction now enqueues once at the post-loop site covering normal and structured-output turns alike. The full suite is **420 passed, 0 failed** (dev fixed the two failures we had been carrying), and a live turn recalled four memories through dev's new completion path.
- [x] **W7 test hardening COMPLETE** — commit `db438c46`. 45 new tests across seven files covering the write gate, ranking maths, decision parsing, the never-raise contracts, round trips, and structural guards against a future merge dropping an import, duplicating the enqueue, losing an identity check or reintroducing a sync route handler. Suite: **465 passed, 0 failed**. Writing them found a real defect: the recall indicator reported entries *selected* rather than entries *rendered*, so it overstated whenever the character budget trimmed — fixed by making `_entry_lines` the shared source of truth.
- [x] **W8 console tenant plumbing COMPLETE** — commit `efbf8444`. The harness already read `x-tenant-id` and accepted `tenant_id` on the turn user object, so the whole gap was console-side: it now reads `DIGIT_CONSOLE_TENANT_ID` server-side, includes the tenant on turns when configured, and proxies all six memory endpoints. Also fixed the standalone console launch, which was not copying static/public assets. Suite 466 passed, 0 failed. **One check outstanding:** the in-pod browser proof is blocked by the corporate access-service intercepting local calls to the console's own API routes — it needs to be done manually through the DevPod proxy URL.

**★ ALL WORKSTREAMS COMPLETE (W0–W8).** Every finding from the production review has shipped code with live receipts.
- [ ] W8 console tenant plumbing + memory UI proxies — split out of W2 (needs npm + browser verification); this is what re-enables memory for console traffic
